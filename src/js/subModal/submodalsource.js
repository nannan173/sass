...
function initPopUp() {
	...
	popcont.innerHTML = '' +
		'<div id="popupInner">' +
			'<div id="popupTitleBar">' +'<div id="popupTitleBar_left">'+'</div>'+
				'<div id="popupTitle">'+title+'</div>' +
				'<div id="popupControls">' +
					'<a onclick="hidePopWin(true);"><span>Close</span></a>' +
				'</div>' +
			'</div>' +
			'<iframe src="'+ gLoading +'" style="width:100%;height:100%;overflow:hidden;background:#F7F7F7;" scrolling="auto" frameborder="0" allowtransparency="true" id="'+popupFrameId+'" name="'+popupFrameId+'" width="100%" height="100%"></iframe>' +
		'</div>';
...
function showPopWin(url, width, height, returnFunc,title,frameId) {
	...
	if(null == gTitle){
		...
		document.getElementById("popupTitle").innerHTML = gTitle;
	}
	gPopupIsShown = true;
	disableTabIndexes();
...
function hidePopWin(callReturnFunc) {
	if (callReturnFunc == true && gReturnFunc != null) {
		if( !gReturnFunc(window.frames[popupFrameId].returnVal)){return false;}
	}
	gPopupIsShown = false;
	restoreTabIndexes();
	if (gPopupMask == null) {
...
function setPopTitleAndRewriteTargets() {
	...
	if (includeDoc.title == null || includeDoc.title=="") {
		...
		document.getElementById("popupTitle").innerHTML = includeDoc.title;		
